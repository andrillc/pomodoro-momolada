...
let timeLeft = 1500;
let timerId = null;
let expectedEndTime = 0; // NOVA VARIÁVEL: Previne o bug do navegador pausar o tempo
...
// Estado das Tarefas Iniciadas do Usuário
let todos = JSON.parse(localStorage.getItem("focus_todos")) || [
  {
    id: "1",
    text: "Aqui você pode definir uma tarefa e organizar por prioridade movendo elas",
    completed: false,
    priority: "high",
  },
];
...
// --- Lógica Principal do Timer Atualizada (Com cálculo de hora real) ---
...
function handleTick() {
  ...
  // Calcula o tempo restante com base no relógio do sistema para nunca atrasar
  const now = Date.now();
...
function renderTodos() {
  ...
  todos.forEach((todo) => {
    ...
    li.querySelector(".checkbox-custom").addEventListener("click", () =>
      toggleTodo(todo.id),
    );
    li.querySelector(".priority-dot").addEventListener("click", () =>
...
// Força a limpeza de resíduos de cache antigos
...
function cyclePriority(id) {
  ...
  todos = todos.map((t) => {
    if (t.id === id) {
      let nextIdx =
        (pLevels.indexOf(t.priority || "medium") + 1) % pLevels.length;
...
document.addEventListener("DOMContentLoaded", () => {
  workDurationInput =
    parseInt(document.getElementById("work-time-input").value) || 25;
  restDurationInput =
    parseInt(document.getElementById("rest-time-input").value) || 5;
  totalTime = workDurationInput * 60;
  timeLeft = totalTime;
  updateDisplay();
  ...
  // Funcionalidade de Play
  document.getElementById("btn-main").addEventListener("click", function () {
    initAudio();
    if (!isRunning) {
      isRunning = true;
      expectedEndTime = Date.now() + (timeLeft * 1000); // Define a meta no tempo real
      this.classList.add("hidden");
      document.getElementById("extra-controls").classList.remove("hidden");
      document.getElementById("btn-pause").innerText = "PAUSAR";
      timerId = setInterval(handleTick, 500); // Ciclo mais rápido para visualização fluida
    }
  });

  // Funcionalidade de Pause/Retomar
  document.getElementById("btn-pause").addEventListener("click", function () {
    if (isRunning) {
      ...
      isRunning = true;
      expectedEndTime = Date.now() + (timeLeft * 1000); // Recalcula a meta ao retomar
      this.innerText = "PAUSAR";
      timerId = setInterval(handleTick, 500);
    }
  });
  ...
  // --- Lógica dos Novos Botões de Setinha para Tempo ---
  function updateInputsLogic(inputId, isWork) {
...
// Permite digitar o tempo diretamente nos campos

  document.getElementById("work-time-input").addEventListener("change", function () {
    let val = parseInt(this.value);
    if (isNaN(val) || val < 1) this.value = 25; // Retorna ao padrão se digitar algo inválido
    updateInputsLogic("work-time-input", true);
  });

  document.getElementById("rest-time-input").addEventListener("change", function () {
    let val = parseInt(this.value);
    if (isNaN(val) || val < 1) this.value = 5; // Retorna ao padrão se digitar algo inválido
    updateInputsLogic("rest-time-input", false);
  });

  // Se os botões de ajuste existirem na interface, escutamos os cliques
  if (document.getElementById("work-minus")) {
      document.getElementById("work-minus").addEventListener("click", () => {
        const input = document.getElementById("work-time-input");
        if (input.value > 1) { input.value = parseInt(input.value) - 1; updateInputsLogic("work-time-input", true); }
      });
      document.getElementById("work-plus").addEventListener("click", () => {
        const input = document.getElementById("work-time-input");
        if (input.value < 480) { input.value = parseInt(input.value) + 1; updateInputsLogic("work-time-input", true); }
      });
      document.getElementById("rest-minus").addEventListener("click", () => {
        const input = document.getElementById("rest-time-input");
        if (input.value > 1) { input.value = parseInt(input.value) - 1; updateInputsLogic("rest-time-input", false); }
      });
      document.getElementById("rest-plus").addEventListener("click", () => {
        const input = document.getElementById("rest-time-input");
        if (input.value < 120) { input.value = parseInt(input.value) + 1; updateInputsLogic("rest-time-input", false); }
      });
  }

  // Função do novo botão de Menu Hambúrguer (Ocultar/Exibir Sidebar de Missões)
